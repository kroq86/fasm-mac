...
int main(void){
 ...
 /* Accumulating kernels require a zero action at each reused gradient
    lifetime. The runtime now schedules five scratch-zero actions internally. */
 unsigned conservative=data_peak+gradient_bytes,executable=theoretical;
 ...
 if(separate_bytes!=452||data_peak!=160||gradient_bytes!=224||conservative!=384||executable!=256)return 1;
 for(uint8_t b=0;b<BUFFER_COUNT;b++)
   printf("%-10s first=%u last=%u scratch_offset=%u\n",names[b],first[b],last[b],slot[b]*64);
 printf("liveness spike passed: separate_bytes=%u conservative=%u executable=%u raw_peak=%u scratch_zero_actions=5 savings=%.2fx\n",
        separate_bytes,conservative,executable,raw_peak,
