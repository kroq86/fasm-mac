...
OUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/tensor-autoregressive-loop.XXXXXX")"
...
for t in 7 8 9; do
  clang -arch x86_64 -std=c11 -O2 -Wall -Wextra -Werror -Wno-unused-parameter -Wno-unused-function \
    -DT="$t" -DH=2 -DD=3 -DM=6 -DQW=18 -DF=8 -I"$ROOT/fasm/spikes" \
    "$ROOT/fasm/spikes/tensor_autoregressive_loop_differential_check.c" "$OUT_DIR/executor.o" -lm -o "$OUT_DIR/check_T$t"
