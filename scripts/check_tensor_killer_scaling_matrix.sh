...
OUT="$(mktemp -d "${TMPDIR:-/tmp}/tensor-killer-scaling.XXXXXX")"; trap 'rm -rf "$OUT"' EXIT
...
clang -arch arm64 -O3 -Wall -Wextra -Werror "$ROOT/fasm/spikes/tensor_killer_scaling_native.c" -o "$OUT/native"
"$PYTHON" "$ROOT/scripts/tensor_killer_scaling_matrix.py" \
  --native "$OUT/native" --python "$PYTHON" --prep "$ROOT/scripts/tensor_killer_scaling.py" \
  --mnist "$MNIST_DIR" --out "$OUT/models" "$@"
