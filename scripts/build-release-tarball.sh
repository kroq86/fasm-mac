...
if [[ ! -x "$ROOT/fasm/build/out/macos-x64/fasm-macos-x64" ]]; then
  echo "Building host binary..." >&2
  "$ROOT/fasm/build/macos-x64.sh"
fi

DEST="$STAGING/$PKG"
mkdir -p "$DEST"

cp -R "$ROOT/fasm" "$DEST/fasm"
cp -R "$ROOT/bin" "$DEST/bin"
cp "$ROOT/README.md" "$ROOT/install.sh" "$ROOT/dotfasm-mac" "$DEST/"
