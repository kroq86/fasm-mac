...
OUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/tensor-gelu.XXXXXX")"
...
clang -arch x86_64 -std=c11 -O2 -Wall -Wextra -Werror -Wno-unused-parameter -Wno-unused-function \
  -I"$ROOT/fasm/spikes" "$ROOT/fasm/spikes/tensor_gelu_differential_check.c" "$OUT_DIR/executor.o" -lm -o "$OUT_DIR/check"
