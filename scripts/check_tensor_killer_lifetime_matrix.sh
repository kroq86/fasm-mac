...
OUT="$(mktemp -d "${TMPDIR:-/tmp}/tensor-killer-lifetime.XXXXXX")"; trap 'rm -rf "$OUT"' EXIT
...
"$PYTHON" "$ROOT/scripts/tensor_killer_lifetime_matrix.py" \
  --native "$OUT/native" --python "$PYTHON" --runner "$ROOT/scripts/tensor_killer_mnist.py" --model "$OUT/model" "$@"
