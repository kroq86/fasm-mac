...
OUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/tensor-merged-mnist-resource-gate.XXXXXX")"
...
clang -arch x86_64 -std=c11 -Wall -Wextra -Werror -Wno-misleading-indentation -O2 \
  "$ROOT/fasm/spikes/tensor_merged_mnist_resource_gate.c" "$OUT_DIR/executor.o" -o "$OUT_DIR/check"
