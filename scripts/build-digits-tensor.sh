...
OUT="${1:-$ROOT/fasm/build/out/digits-tensor-train}"
OBJ_DIR="$(mktemp -d "${TMPDIR:-/tmp}/digits-tensor-build.XXXXXX")"
...
clang -arch x86_64 -O2 -c "$ROOT/fasm/examples/digits_tensor_train.c" -o "$OBJ_DIR/digits_tensor_train.o"
clang -arch x86_64 "$OBJ_DIR/digits_tensor_train.o" "$OBJ_DIR"/tensor_*.o -o "$OUT"
chmod +x "$OUT"
printf 'Built %s\n' "$OUT"
