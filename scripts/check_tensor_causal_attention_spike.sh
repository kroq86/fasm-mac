...
OUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/tensor-causal-attention.XXXXXX")"
...
for t in 7 8 9; do
  clang -arch x86_64 -std=c11 -O2 -Wall -Wextra -Werror -Wno-unused-parameter -Wno-unused-function \
    -DT="$t" -I"$ROOT/fasm/spikes" "$ROOT/fasm/spikes/tensor_causal_attention_differential_check.c" "$OUT_DIR/executor.o" -lm -o "$OUT_DIR/check_T$t"
