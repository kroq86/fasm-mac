...
STAGING="$(mktemp -d)"
PKG="raymaze-${VERSION}-macos-x86_64"
OUT_DIR="$ROOT/dist"
...
if ! RAYLIB_FLAGS="$(raylib_flags)"; then
  echo 'raymaze release requires an x86_64 raylib; use an Intel/Rosetta Homebrew raylib install' >&2
  exit 1
fi
...
OBJ="$STAGING/raymaze.o"
fasm --emit=macho-obj "$ROOT/fasm/apps/raymaze.asm" "$OBJ" >/dev/null
clang -arch x86_64 "$OBJ" $RAYLIB_FLAGS -o "$DEST/raymaze"
chmod +x "$DEST/raymaze"
