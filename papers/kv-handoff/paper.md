...
Transferring a language model's key–value cache can change another model's predictions without establishing that the receiver can use the transferred information. We report a retrospective case study of frozen GPT-2 and DistilGPT2 connected by task-specific rank-8 residual KV adapters with 147,456 trainable parameters. An order-dependent binary classification checkpoint achieves 31/32 correct predictions on its original development set; an opposite-class cache replacement reduces accuracy to 1/32. A separately trained five-row lookup adapter reports 20/32 exact two-token answers on a held-out test split, compared with 1/32 under document replacement and 1/32 under binding permutation. Conversely, an abstract two-key task has zero top-1 accuracy despite a positive likelihood contrast. An audit of the attempted XOR recombination test finds that all recombined targets belong to one class, preventing its interpretation as a balanced composition test. These experiments do not isolate the cause of the between-task differences. We additionally reproduce the saved ordering checkpoint in an independent native execution path: selected sender caches, adapted caches, and receiver logits agree with a CPU reference within a fixed numerical tolerance on all 32 cases, including the same classification error. The report contributes a bounded, inspectable reproduction and an artifact-linked account of successes and limitations, not a new cache-transfer method, evidence of general reasoning, or a demonstrated efficiency advantage.
...
| XOR / separately trained XOR adapter | 32 original dev inputs; 32 recombined inputs all labeled 1 | Stored 22/32 original; 14/32 recombined | Constant-one baseline is 32/32 on recombined inputs | Stored score; data-design defect independently reproduced | Invalid as a balanced composition test; generalization unresolved |
...
The historical score and interval are retained, not recomputed or erased. The stored run records 800 epochs, development-accuracy selection and best checkpoint epoch 180; current training-budget defaults in the script have since changed and are not a pinned reproduction recipe for that historical training run. The pairing defect follows directly from the selection and recombination functions, independently of those budget defaults. We retract the interpretation of 14/32 as a clean negative result on a balanced compositional-generalization test: that intended test is invalid, and the broader generalization claim remains unresolved. `python3 papers/kv-handoff/check_xor_design.py` reproduces the design diagnosis without model inference or training. No repaired test was run for this report.
...
| Composition / cache merging | XOR recombination creates only class-1 targets; its balanced-test interpretation is invalid, not a clean negative capability result. Ready-made cache concatenation, multiple senders and incremental updates are not tested here. |
