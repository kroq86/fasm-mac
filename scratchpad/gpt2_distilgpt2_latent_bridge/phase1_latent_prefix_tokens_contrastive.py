"""Contrastive-objective rerun: same norm-calibrated 8-token latent-prefix
bridge (phase1_latent_prefix_tokens_norm_calibrated.py), exactly one
change -- the training loss.

That run fixed the catastrophic-destabilization confound and got a clean
negative on the content-specific gate: `correct` NLL 2.636 vs `shuffled`
2.633 (CI [-0.007422, 0.000598], includes zero) and vs `no_prefix` 2.626
(CI [-0.025681, 0.003782], includes zero), while `correct` significantly
beat `zero_sender` 2.669 (CI [0.017879, 0.046842], excludes zero) -- the
same pattern as the very first scale+bias rung: a real, significant gain
over a true no-content baseline that is not document-specific. Every prior
rung (this one included) only ever minimized NLL on the correct document
and checked the correct-vs-shuffled gap *after the fact*; nothing was ever
explicitly incentivized to make the correct document's state outperform a
wrong document's state during training. That is a genuinely different
axis (training objective) from the three already closed (bridge capacity,
injection site, fusion mechanism).

Claim: adding an explicit contrastive hinge term to the training loss,
`max(0, margin - (NLL_shuffled - NLL_correct))` with a fixed
`margin=0.05`, `lambda=1.0` (equal weight with the NLL term; chosen before
running, matching the scale of gaps already observed in this project, not
tuned after seeing a result), will produce a bridge whose correct-vs-
shuffled gap on held-out dev is significant, where pure NLL minimization
did not. Falsified by: correct still indistinguishable from shuffled on
dev even under direct optimization pressure to separate them -- which
would be a materially stronger negative than every prior rung, since it
rules out "wasn't incentivized" as an explanation.

Total per-step loss: `NLL(correct) + lambda * relu(margin - (NLL(shuffled) - NLL(correct)))`,
computed per training document against its i-1-rotated neighbor (same
shuffled-pairing convention used for the dev-time shuffled arm throughout
this project). Everything else -- corpus, splits, site (GPT-2 final layer
-> DistilGPT2 first block), K=8 latent tokens, linear no-bias projector,
norm calibration to the receiver's real median embedding norm, budget,
dev-arm definitions, primary/secondary gates -- is unchanged. If this also
fails to separate correct from shuffled, the latent-bridge line at this
model pair/scale is closed without further tuning, per the project's stop
rule: the next candidate confound (model scale) is not cheaply testable
here.
"""
...
def main() -> int:
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for i, d in enumerate(train):
            ...
            z = bridge(d["H"])
            logits = receiver_logits(receiver, wte, d["suffix_ids"], z)
            nll_correct = scored_ce(logits, target)

            other_H = train[(i - 1) % n_train]["H"]
            z_shuf = bridge(other_H)
            logits_shuf = receiver_logits(receiver, wte, d["suffix_ids"], z_shuf)
            nll_shuffled = scored_ce(logits_shuf, target)

            hinge = torch.relu(CONTRASTIVE_MARGIN - (nll_shuffled - nll_correct))
            loss = nll_correct + CONTRASTIVE_LAMBDA * hinge
            loss.backward()
            opt.step()
            epoch_losses.append(loss.item())
            epoch_nll_correct.append(nll_correct.item())
            epoch_hinge.append(hinge.item())
        train_loss_history.append(sum(epoch_nll_correct) / len(epoch_nll_correct))
        hinge_history.append(sum(epoch_hinge) / len(epoch_hinge))
    ...
    result = {
        ...
        "bridge": "linear_pooling_prefix_tokens_norm_calibrated_contrastive_prepended_to_input_embeds",
        ...
        "finite": finite,
        "verdict": (
            "LATENT_PREFIX_CONTRASTIVE_TRAINING_INCONCLUSIVE" if still_improving else
            "LATENT_PREFIX_CONTRASTIVE_DOCUMENT_SPECIFIC_TRANSFER" if signal else
            "LATENT_PREFIX_CONTRASTIVE_NO_DOCUMENT_SPECIFIC_TRANSFER"
        ),
        "per_document_dev": {
