#!/usr/bin/env python3
"""Adapter-rank sweep for the natural-language lookup variant, on two seeds
(1, 2) that phase1_kv_cache_sql_lookup_binding_natural_language_seed_
variance_result.json found collapse to 0/32 exact accuracy at the arrow-
table recipe's original rank=8, LR=1e-2.

Motivation: the natural-language document is ~20% longer in tokens than
the arrow-table format (31 vs 26 tokens for the same 5-name table), a
plausible capacity explanation for the collapse -- maybe rank=8 is not
enough to route the extra tokens. This sweep (rank in {8, 16, 32, 64}, LR
held at the original 1e-2) rules that out: exact accuracy stays at 0/32 at
every rank, and NLL gets slightly WORSE at higher rank (more parameters,
same or worse result -- the opposite of what a capacity bottleneck would
predict). This motivated the LR sweep instead (phase1_kv_cache_sql_lookup_
binding_natural_language_lr_sweep.py), which found the real cause.
"""
...
def train_and_eval(sender, receiver, names, target_k, n_head, head_dim, n_layers,
                    ...
                    swap_q, swap_tgt, adapter_seed: int, rank: int) -> dict:
...
def main() -> int:
    ...
    runs = []
    for seed in [1, 2]:  # two known 0/32 failures at rank=8
        for rank in [8, 16, 32, 64]:
            r = train_and_eval(sender, receiver, names, target_k, n_head, head_dim, n_layers,
                                train_k, train_v, train_q, train_tgt, dev_k, dev_v, dev_q, dev_tgt,
                                test_k, test_v, test_q, test_tgt, wrong_k, wrong_v,
                                swap_q, swap_tgt, seed, rank)
            r["rank"] = rank
            print(f"seed={seed} rank={rank:>2} exact_accuracy={r['exact_accuracy']:.4f} "
    ...
    result = {
        ...
        "method": (
            ...
            "the arrow-table recipe's original rank=8, LR=1e-2 on the "
            "natural-language document wording, LR held at 1e-2, adapter "
            "rank swept over {8, 16, 32, 64}. Same fixed train/dev/test "
            "split and natural-language wording as phase1_kv_cache_sql_"
            "lookup_binding_natural_language_seed_variance.py throughout."
        ),
        "seeds_tested": [1, 2],
        "ranks_tested": [8, 16, 32, 64],
        "runs": runs,
    }
