#!/usr/bin/env python3
"""Multi-fact KV-cache addressing, K=2 keys, query-conditioned K mapping.

Two prior fixes both failed identically: a shared per-layer K/V adapter
(`wrong_key_minus_correct` CI `[-0.316, 0.312]`) and a per-position K
adapter with 1.1M params (`[-0.320, 0.319]`) -- giving K positional
freedom alone did not help at all. Refined hypothesis: attention retrieves
via a Q<->K match; the adapter never sees the receiver's actual query, so
it can only learn "this cache is useful in aggregate", never "these K
positions should respond to a Person1 query, those to a Person2 query".

This script makes K's transform depend on which of the two fixed queries
will be used. Since the query text is one of two known, constant strings,
its "fingerprint" is precomputed once from the receiver's own embedding
layer (`wte(query_ids).mean(dim=1)`, no receiver internals hooked beyond
the public embedding table) -- not trained, just a fixed conditioning
signal available before the receiver ever runs. K's correction is now
GATED by this fingerprint, per position: `K'_p = K_p + gate_p(query_fp) *
correction_p`, `gate_p = sigmoid(query_fp @ W_gate[:, p])`, a small
per-layer (768, prefix_len) matrix -- far fewer new parameters than the
failed per-position experiment (~69K vs 1.1M), testing a qualitatively
different mechanism (query-dependent modulation) rather than raw
capacity. V keeps the original single shared (A,B) pair per layer,
unconditioned, unchanged from both failed runs (addressing is hypothesized
to live in K, not V). Base K correction (before gating) is still a single
shared (A,B) per layer, matching the very first (failed) design -- only
the NEW per-position gate is added on top.

Same task/controls/gate as both prior 2-key scripts: correct (diagonal)
must beat wrong_key (off-diagonal) with CI excluding zero, AND beat
shuffled_document with CI excluding zero. If this also fails, the next
step (not this script) is cross-attention over the sender's raw KV -- a
genuinely different retrieval mechanism, not another adapter variant.
"""
...
class QueryConditionedKVAdapter(nn.Module):
    """K: shared per-layer (A,B) low-rank correction, GATED per position by
    a query fingerprint (sigmoid(query_fp @ W_gate) -> (batch, prefix_len)).
    V: original unconditioned shared (A,B) per layer, untouched.
    B (for K and V) is zero-initialized -> identity/raw baseline at init,
    regardless of gate value or query -- the gate's own init doesn't need
    ...
    def __init__(self, n_layers: int, prefix_len: int, head_dim: int, n_head: int, rank: int, seed: int):
        ...
        self.W_gate = nn.ParameterList([nn.Parameter(torch.randn(width, prefix_len, generator=g) * GATE_INIT_STD) for _ in range(n_layers)])
    ...
    def _correction(self, x: torch.Tensor, A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
        b, h, s, d = x.shape
        flat = x.permute(0, 2, 1, 3).reshape(b * s, h * d)
        correction = (flat @ A) @ B
        return correction.reshape(b, s, h, d).permute(0, 2, 1, 3)

    def forward_keys(self, keys: list[torch.Tensor], query_fp_batch: torch.Tensor) -> list[torch.Tensor]:
        out = []
        for i, k in enumerate(keys):
            corr = self._correction(k, self.A_k[i], self.B_k[i])  # (batch, n_head, seq, head_dim)
            gate_logits = query_fp_batch @ self.W_gate[i]  # (batch, prefix_len)
            gate = torch.sigmoid(gate_logits).reshape(k.shape[0], 1, self.prefix_len, 1)
            out.append(k + gate * corr)
        return out

    def forward_values(self, values: list[torch.Tensor]) -> list[torch.Tensor]:
        out = []
        for i, v in enumerate(values):
            corr = self._correction(v, self.A_v[i], self.B_v[i])
            out.append(v + corr)
        return out
...
def compute_query_fingerprints(receiver, tokenizer) -> torch.Tensor:
    """(2, width) -- one fixed fingerprint per query slot, from the
    receiver's own token embeddings (mean-pooled), not trained."""
    wte = receiver.transformer.wte
    fps = []
    with torch.no_grad():
        for slot in range(2):
            ids = query_ids_for(tokenizer, slot, 1)
            fps.append(wte(ids).mean(dim=1).squeeze(0))
    return torch.stack(fps, dim=0)  # (2, width)
...
def build_training_batch(keys, values, target, tokenizer, n_docs: int, query_fps: torch.Tensor):
    k2 = repeat_kv(keys, 2)
    v2 = repeat_kv(values, 2)
    q0 = query_ids_for(tokenizer, 0, n_docs)
    q1 = query_ids_for(tokenizer, 1, n_docs)
    query = torch.cat([q0, q1], dim=0)
    tgt = torch.cat([target[:, 0], target[:, 1]], dim=0)
    fp_batch = torch.cat([query_fps[0].unsqueeze(0).expand(n_docs, -1), query_fps[1].unsqueeze(0).expand(n_docs, -1)], dim=0)
    return k2, v2, query, tgt, fp_batch
...
def main() -> int:
    ...
    query_fps = compute_query_fingerprints(receiver, tokenizer)  # (2, width)
    ...
    adapter = QueryConditionedKVAdapter(n_layers, prefix_len, head_dim, n_head, RANK, seed=SEED).to(DEVICE)
    query_fps = query_fps.to(DEVICE)
    opt = torch.optim.Adam(adapter.parameters(), lr=LR)
    print(f"prefix_len={prefix_len} trainable_params={adapter.param_count()}")

    tk2, tv2, tquery, ttgt, tfp = build_training_batch(train_k, train_v, train_target, tokenizer, n_train, query_fps)
    k0 = adapter.forward_keys(tk2, tfp)
    ...
    def eval_arm(keys, values, slot: int, target_col: int) -> tuple[list[float], list[float]]:
        batch = keys[0].shape[0]
        q = query_ids_for(tokenizer, slot, batch)
        fp = query_fps[slot].unsqueeze(0).expand(batch, -1)
        with torch.no_grad():
            k = adapter.forward_keys(keys, fp)
    ...
    def dev_loss() -> float:
        adapter.eval()
        with torch.no_grad():
            k2, v2, query, tgt, fp = build_training_batch(dev_k, dev_v, dev_target, tokenizer, n_dev, query_fps)
            k = adapter.forward_keys(k2, fp)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        opt.zero_grad()
        tk2, tv2, tquery, ttgt, tfp = build_training_batch(train_k, train_v, train_target, tokenizer, n_train, query_fps)
        k = adapter.forward_keys(tk2, tfp)
    ...
    result = {
        ...
        "n_keys": 2, "names": list(NAMES), "prefix_len": prefix_len,
        ...
        "mechanism": "query_conditioned_gated_lowrank_kv_adapter_2key_addressing",
        "dev": {
        ...
        "verdict": (
            "KV_2KEY_QUERYCOND_TRAINING_INCONCLUSIVE" if still_improving else
            "KV_2KEY_QUERYCOND_ADDRESSING_SUPPORTED" if signal else
            "KV_2KEY_QUERYCOND_ADDRESSING_NOT_SUPPORTED"
        ),
    }
