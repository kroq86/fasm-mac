...
Same architecture as the 1-token/2-token single-fact KV scripts: GPT-2's
KV-cache at layers (1,3,5,7,9,11) transplanted into DistilGPT2 via
past_key_values, per-layer per-K/V low-rank residual adapter (rank 8, no
bias, B zero-initialized), full-batch training (established as the
categorically better regime for this adapter class).
"""
...
def main() -> int:
    ...
    result = {
        ...
        "mechanism": "low_rank_residual_kv_adapter_batched_2key_addressing",
        "dev": {
        ...
        "verdict": (
            "KV_2KEY_TRAINING_INCONCLUSIVE" if still_improving else
            "KV_2KEY_ADDRESSING_SUPPORTED" if signal else
            "KV_2KEY_ADDRESSING_NOT_SUPPORTED"
        ),
    }
